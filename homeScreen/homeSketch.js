...
function preload() {
  ...
  tempArray.push(loadImage("img/targetImage6.jpeg"));
...
function setup() {
  ...
  for (var y = 0; y < targetImages[imageNo].height; y += 5) {
    for (var x = 0; x < targetImages[imageNo].width; x += 5) {
      ...
      if (
        targetImages[imageNo].pixels[index + 0] == 0 &&
        targetImages[imageNo].pixels[index + 1] == 0 &&
        targetImages[imageNo].pixels[index + 2] == 0
      ) {
...
function draw() {
  if (!pause) {
    ...
    let adder = createVector(
      mouseX - previousPosition.x,
      mouseY - previousPosition.y
    );
    ...
    middleMousePos = createVector(
      (mouseX + previousPosition.x) / 2,
      (mouseY + previousPosition.y) / 2
    );
